...
for the list; the acceptance test holds (`Unrect` references nothing in `Unrect.Engine`), and §5.4
landed too (`2ba3f0d`): the strategy calculus is single-form — each contract one `Begin`, the scan
the strategy's own, the whole-region answer the fold — so nothing of the pull interpreter runs.

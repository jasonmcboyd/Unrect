...
> **Status: built** (2026-09-17). What shipped, and where it deviates from this section, is
> `docs/design/typed-predicate-lift.md`.
...
**Phase‑6 rulings (2026‑09‑15; amend §2).** (i) `IProjection<TSpace, TResult>` is **invariant**: `in TSpace` and a real `Project(Plane<TSpace>, …)` are mutually exclusive (`Plane<TSpace>` is an invariant struct, so `TSpace` in that parameter must be invariantly valid — CS1961 — and a struct cannot be declared variant). Option A chosen over splitting the plane into `(space, Region)` or passing an erased extent with a `Retype`: the plane stays one value at the boundary, shared helpers are generic methods instantiated at the caller's `TSpace` (which need no variance), nothing in the tree passes a projection *value* across two spaces, and a `Retype` would reverse erasure. `Demand`/UNR001 still retire (their reason was lexical `TSpace`, not variance); the §15 strategy/landmark phantoms keep `in` (no plane in their signatures). UNR003's documented trigger becomes the composition‑site failure (CS1503/CS0311 at `v.Next`/`Choice`/`Else`) as well as CS0411 at `Map`. (ii) `ProjectionBuilders<TSpace>` carries one constraint, `where TSpace : class, ICellValues`, for phases 6–10 (every shipped space is one; the canonical members need only `ISpace`); relaxed in phase 11 when the kinded members leave. (iii) The engine finds `ISweepAware` by a type test on the root space; `Workbook.Sheet`'s return type is untouched until phase 7.

**Re‑cut (owner, 2026‑09‑15: "Go").** Green‑per‑commit is no longer required for the rest of the arc; WIP commits are allowed. Nothing scheduled for deletion is genericised or copied: **phases 7–12 fold into phase 6's four checkpoints** — (1) `Unrect` compiles with no reference to `ICellValues`/`CellValue` (the kinded leaves, binder, `CellReading`, `Table<T>` rungs, `Cell<T>`, the 36 accessors and `TableRow.Cells`/`TryGet` deleted outright; `Point()`, `AsText()`, `GridSpace<T>`/`IValueCells<T>`/`Value()`, views over points, `Table()`/`Fields` yielding points, the read‑failure protocol added); (2) `Spreadsheets` compiles and Core is final (`CellValue` *moves* to `Spreadsheets` as `Cell`, `ICellValues` deleted; `ISheetCells`, `SheetGrid`, point reads, the six leaves as real primitives, `Record<T>`/`Table<T>` composed, the capability seam deleted, `ISweepAware`; the streaming gate runs here); (3) tests re‑scoped and relocated in one pass; (4) analyzers. Phase 13 (surfaces) unchanged. The §15 typed‑predicate lift is deferred as an additive step after the tree is green. Cost accepted: one red window for the campaign, reviewed and gated per checkpoint rather than per commit.

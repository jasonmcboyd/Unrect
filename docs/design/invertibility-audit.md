...
| **4** | **Unify the matcher** (§4.1): one `IRowMatch`/`IColumnMatch`, three lifts (`To`, `Past`, `RowsBefore`), `.Until` unchanged; fix the `Where`/`WithCell` name crossing | small design | Prerequisite for 5; kills the `SkipRows(1)` idiom; gives anchors a description. |
| **5** | **`Label`/`Caption` leaves** (C4) | small design | Makes anchor rows *declared content*; removes duplicate literals in two scripts; the first place `read ∘ write` becomes satisfiable. |

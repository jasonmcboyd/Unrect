...
| **6** ✅ | **Declared table columns** (C2) — **done** (`typed-leaves-and-tables-spec.md` phase C): `TableRows<T>` binds captions to members by name, with per-member overrides and `Ignore`. By-name table bindings in the corpus fell 27 → 7, all in the one deliberately-preserved lambda example. | medium design | The largest single cleanup in the corpus (27 sites). Already deferred-and-planned; the audit's evidence says promote it. Decide expression-trees vs. a column cursor. |
| **7** ✅ | **Typed cell leaves** (C1) — **done** (same spec): six leaves closed over `CellValue`'s accessor set behind the firewall rule. Corpus accessor calls fell 53 → 22. | medium design | 61 sites; falls out of the same binding notion as 6 and should share its spelling. Big diagnostics win independent of writers. |
| **8** ✅ | **`Fields` / labelled-pair block** (C6) — **done** (same spec): the K-1 entity card is five `Field` declarations, self-anchored, with the hard-coded `2, 5` and the `TrimEnd(':')` both gone. C7 fell out with it, via `CellValue` as a legal member type. | medium | Built on 7. Also removes a hard-coded `2, 5` that CLAUDE.md's own rule condemns. |

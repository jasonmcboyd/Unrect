...
| header of several rows | `Table(2, r => r["From", "Id"].Integer())` | `Unrect` | as the rung it is said on | `headerRows` is any count: the last row is the captions and the rows above it are BANDS (a merged "From" over Id and Code). A column is addressed by its PATH — a name, the nth of a name `("Id", 1)`, or a position `1`, every number zero-based, never joined into text. A bare name is the column whose whole path it is, before a caption unique under a band; one that several columns answer to is refused with the paths that say which. Bands reach rightward by convention (a label claims the blanks beneath it, else those to its right, stopping at a spacer); a label is whatever a header cell SAYS, so a row of years is a row of captions |
...
| reflective, adjusted | `Table<T>(bind => ...)` | `Unrect.Spreadsheets` | `IReadOnlyList<T>` | The same, adjusted: `bind.Column(t => t.X, "caption")` for a caption the comparer would not find, `Table<T>(headerRows, bind, onBlank)` over a banded header, where FLAT binding is the rule — a member answers to a caption, or to the banded column whose whole path run together is its name (`FromId` for `From, Id`), and `bind.Column(t => t.X, "From", "Id")` says which where two answer; `bind.Column(t => t.X, row => ...)` for a member filled from the ROW rather than a column (what a cell looks like, two columns compared — `row => row["Fund"].Font().Color == CellColor.Red`; any type, no caption asked for), `bind.Column(t => t.X, 3)` for a column a caption cannot reach (a blank header cell, or one of two captions that say the same thing — the position counts from the table's left edge, as `r[3]` does), `bind.Ignore(t => t.X)` for a member this table does not carry |

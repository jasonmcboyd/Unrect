...
{
  /// <summary>
  /// SPIKE. A space that remembers how far down the sheet the reading has got, and how far back
  /// behind that it ever reached. A windowed reader is only as cheap as the walk over it is
  /// monotone, so this is the instrument that says whether a declaration would survive one — with
  /// no streaming machinery involved, and therefore nothing to argue with.
  /// <para>
  /// Subspaces carry the row translation, so every reach is reported in the outermost space's own
  /// coordinates.
  /// </para>
  /// </summary>
  public sealed class WatermarkSpace : ISpace
  {

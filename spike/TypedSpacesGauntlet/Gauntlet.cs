...
namespace TypedSpacesGauntlet
{
  ...
  public static class Gauntlet
  {
    ...
    // type is IShape<IReadOnlyList<SourcedAllocation>> — plain. Nothing acquired a demand, so
    ...
    public static IReadOnlyList<SourcedAllocation> ProjectionUnchecked(ISpace sheet)
    {
      ...
      IShape<IReadOnlyList<SourcedAllocation>> declared = rows;   // still plain: no demand inferred
    ...
    public static IReadOnlyList<SourcedAllocation> ProjectionAscribed(FormulaGridSpace sheet)
    {
      ...
      IShape<IFormulaSpace, IReadOnlyList<SourcedAllocation>> declared = rows;
    ...
    public static IShape<IFormulaSpace, IReadOnlyList<SourcedAllocation>> ProjectionAscribedByTypeArgs()
    ...
    // =============================================================================================
    //
    // ZERO annotations. The receiver is a plain shape and the matcher is IRowLandmark<IFormulaSpace>;
    // inference unifies the two upper bounds to the more demanding one, so the modifier chain hands
    // back a demanding shape on its own.
    public static string FirstFormulaRow(FormulaGridSpace sheet)
    {
      ...
      IShape<IFormulaSpace, string> declared = firstFormulaRow;   // the inferred type, spelled out
    ...
    // =============================================================================================
    //
    // Here is where the tax lands, and it lands hard: a lambda body cannot drive inference, so BOTH
    // type arguments must be written on the flow — the demand (which is a statement worth making)
    // and the result type (which is pure ceremony, and which the plain spelling infers).
    public static AuditedReport MixedComposite(FormulaGridSpace sheet)
    {
      ...
      var report = VerticalFlow<IFormulaSpace, AuditedReport>(v => new AuditedReport(   // TAX x2
        Title: v.Next(title),
        Rows: v.Next(rows),
    ...
    // 4b — the same flow with the demand carried by a WITNESS instead of by type arguments. Zero
    // explicit type arguments: TSpace comes from `Formulas`, T from the lambda's return, exactly as
    // in the plain spelling. This is the best spelling found for the known-hard inference spot, and
    // it turns the scenario-4 tax from "two arguments, one of them ceremony" into "one word".
    public static AuditedReport MixedCompositeWitnessed(FormulaGridSpace sheet)
    {
      var title = Text();
      var rows = TableRows<Allocation>();
      var totalFormula = Formula().On(RowContaining("Total")).Right(2);
    ...
    //
    //   (d) IShape<AuditedReport> laundered = report;
    //       CS0266: Cannot implicitly convert type
    //               'Unrect.Shapes.IShape<Unrect.Spreadsheets.IFormulaSpace, ...AuditedReport>' to
    //               'Unrect.Shapes.IShape<...AuditedReport>'.
    //               An explicit conversion exists (are you missing a cast?)
    //
    //   (f) Choice(Text().Select(t => (string?)t), Formula())  assigned to IShape<string?>
    //       CS0266: ... IShape<IFormulaSpace, string> ... to 'IShape<string?>' ...
    ...
    //
    //   (g) VerticalRepeat(Formula()) assigned to IShape<IReadOnlyList<string?>>
    //       CS0266: ... IShape<IFormulaSpace, IReadOnlyList<string>> ... — repeat unified too.
    //
    //   (h) Row(...).On(RowWithFormula()) assigned to IShape<string>
    //       CS0266: ... IShape<IFormulaSpace, string> ... to 'IShape<string>' ...
    ...
    //               'ShapeExtensions.Map<TSpace, TResult>(IShape<TSpace, TResult>, TSpace)'
    ...
    //       CS0411: The type arguments for method 'LayoutCursor.Next<T>(IShape<T>, string?)'
    //               cannot be inferred from the usage. Try specifying the type arguments explicitly.
    ...
    public static bool DemandCanBeCastAway()
    {
      ...
      var laundered = (IShape<string?>)demanding;
    ...
    /// the other IS the variance direction the design rests on. Expect <c>plain -> demanding: True</c>
    /// and <c>demanding -> plain: False</c>.
    /// </summary>
    public static string VarianceDirection()
    {
      var plain = typeof(IShape<AuditedReport>);
      var demanding = typeof(IShape<IFormulaSpace, AuditedReport>);
    ...
    // =============================================================================================
    // Scenario 7 — hoisted shape libraries: what the declared types look like now.
    // =============================================================================================
    //
    // A plain helper is EXACTLY as it was. This is the common case and it does not move.
    public static IShape<Allocation> AllocationRow()
    ...
    public static IShape<IFormulaSpace, SourcedAllocation> SourcedRow()
    ...
    public static IShape<TSpace, IReadOnlyList<T>> Sections<TSpace, T>(IShape<TSpace, T> item)   // TAX x2 + constraint
    ...
    public static void Run()
    {
      ...
      Show("4  mixed composite (type args)   ", MixedComposite(formulaSheet));
      Show("4b mixed composite (witness)     ", MixedCompositeWitnessed(formulaSheet));
    ...
    private static IShape<string> FirstFormulaRowUntyped()
      => (IShape<string>)Row(cells => cells[0].GetString()).On(RowWithFormula());

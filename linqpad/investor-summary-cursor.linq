...
// The CURSOR spelling of investor-summary.linq — same declaration, same output, for judging
// the style side by side. One lambda both declares each child (in flow order) and consumes
// its value; members bind by name where the applicative spelling threads a tuple.
...
var investorDetail =
	Vertical(v => new
	{
		...
		Transactions = v.Next(TableRows(r => new
		{
...
var report =
	Vertical(v => new
	{
		ReportHeader = v.Next(Column(c => new
		{
			Title = c[0].GetString(),
			ReportDate = c[1].GetDateTime(),
			ReportId = c[2].GetString(),
		}).Named("report header")),
		Summary = v.Next(TableRows(r => new
		{
		...
		Details = v.Next(Repeat(investorDetail, separatedBy: BlankRows(), atLeast: 1)
			.AfterBlankRows()

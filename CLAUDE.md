...
| **Unrect** | The shape layer (`Unrect.Shapes`): the `Shape` vocabulary, `IShape<T>`, `ShapeEngine`, the composites and primitives, the cell views (`CellStrip`/`CellBlock`/`TableView`), and diagnostics — plus, in the root `Unrect` namespace, `GridSpace` (a `CellValue[,]` viewed as an `ISpace`, entered by constructor for cells already canonical or by `Create<T>(values, map)` and the primitive overloads with blank predicates, which is where blankness is decided) and `SpaceExtensions`; and the shared `Orientation` enum with the `CallerArgumentExpressionAttribute` polyfill (netstandard2.1 has no built-in one; it backs use-site name capture) |
| **Unrect.Strategies** | Strategy implementations for computing sizes, offsets, rows, and columns |
| **Unrect.Spreadsheets** | `SpreadsheetSpace` — reads spreadsheet files (`.xls`/`.xlsx` via ExcelDataReader) and adapts cells to `CellValue` eagerly, the whole sheet at once. `Workbook.Open(path)` is the streaming door onto the same files: `Sheet(name)` vends a lent `ISpace` view backed by a windowed chunk store and a lead/chase reader pool, so a shape reads a sheet a window at a time instead of the whole grid. Named for the family, not the vendor: further formats belong here rather than in a second package. |
| **Unrect.Benchmarks** | The continuous-benchmark suite (BenchmarkDotNet, 41 benchmarks in seven one-class families including `Streaming`; not packable). Conventions and the change-judging workflow: `docs/benchmarking.md`. |
...
Streaming Part 1 (`docs/design/streaming-spec.md`) has landed: `Workbook`/`WorkbookOptions` in `Unrect.Spreadsheets`, a windowed `SheetStore` behind a lead/chase `ReaderPool`, `StreamingStatistics`/`ReaderPoolStatistics` as the honest cost vocabulary, and the IO-fault-discipline fix — `ShapeException.IsProjectionFault` renamed `IsFault` and extended to `IOException`/`ObjectDisposedException`/`OutOfMemoryException` at all four sites that wrap a foreign exception, so a disk failure can never be absorbed by `.Optional()`/`.Else()`/`Choice` as an absent section. Part 2 (lazy extents — an incremental row/size/area strategy calculus, additive in Core, so a fixed-width declaration never forces a bound's height before the projection has consumed it) is specced in the same document (§11) but not built; it is gated on Part 1's merge and droppable after any of its eight steps.

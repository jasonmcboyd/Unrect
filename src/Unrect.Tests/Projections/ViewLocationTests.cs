...
namespace Unrect.Tests.Projections
{
  ...
  public class ViewLocationTests
  {
    ...
    {
      // Padding shifts where the inner projection reads, so it must shift what the inner projection
      // reports.
    ...
    public void AddressesSurviveEveryLayerAtOnce()
    {
      ...
      var address = VerticalFlow(v => $"{v.Next(band)}|{v.Next(Overlay(o => $"{o.Next(padded)}/{o.Next(corner)}"))}")
        .Map(CoordinateGrid());

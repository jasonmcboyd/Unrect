...
namespace Unrect.Tests.Projections
{
  ...
  /// <para>
  /// Everything reflective resolves when the projection is built, so a type that cannot be bound is
  /// an error at the declaration rather than once per file. Everything about the <em>data</em> is
...
#nullable restore
    ...
    {
      // Real reports carry columns a consumer does not want; binding is strict in one direction
      // only.
      var space = Mixed(new object?[,]
      {
    ...
    public void OneTypedTableIsSafeToMapFromManyThreads()
    {
      ...
      {
        var row = projection.Map(spaces[index]).Single();

...
namespace Unrect.Tests.Projections
{
  ...
  /// reads it. <c>Decimal()</c> says "a number lives here" where <c>Cell(c =&gt;
  /// c.GetDecimal())</c> only said "read this somehow, and hope".
  /// <para>
  /// The discipline these tests exist to protect is §1.4's: a <em>kind</em> failure speaks kinds
  /// ("expected Number, found Text") and a <em>conversion</em> failure speaks conversions ("is not
  /// a whole number"). A reader who sees the first goes to the column; a reader who sees the second
  /// goes to the cell.
  /// </para>
  /// </summary>
  public class TypedLeafTests
  {
    ...
    {
      Assert.Equal("expected Text at A1, found Number", Problem(Assert.Throws<ProjectionException>(() => Text().Map(One(1)))));
    }
    ...
    public void AKindFailureNeverMentionsTheLeafsOwnName()
    {
      ...
      var failure = Assert.Throws<ProjectionException>(() => Decimal().Map(One("x")));
    ...
    public void IntegerOverANonWholeNumber_SaysSo()
    {
      Assert.Equal(
        ...
        Problem(Assert.Throws<ProjectionException>(() => Integer().Map(One(1.5)))));
    }
    ...
    public void IntegerOutOfRange_SaysSo()
    {
      Assert.Equal(
        ...
        Problem(Assert.Throws<ProjectionException>(() => Integer().Map(One(5e9)))));
    }
    ...
    public void DecimalOverAnUnrepresentableNumber_SaysSo()
    {
      Assert.Equal(
        ...
        Problem(Assert.Throws<ProjectionException>(() => Decimal().Map(One(1e30)))));
    }
    ...
    public void AConversionFailureIsAboutTheCellNotTheColumn()
    {
      ...
      var failure = Assert.Throws<ProjectionException>(() => Integer().Map(One(1.5)));
    ...
    {
      // The payoff the audit predicted. A Choice over typed leaves picks the arm whose kind
      // matches, which the untyped Cell(c => …) spelling could only do by throwing from inside a
      // lambda.
    ...
    public void TheLeafNamesDoNotShadowTheKeywordAliases()
    {
      ...
      // are imported here as static members of Projection; if any of them ever shadowed the
      // framework type of the same name, the declarations and calls below would stop resolving.
      Decimal money = 1m;
      Double ratio = 0.5;
      Boolean flag = true;
      Int32 count = 2;
      ...
      // ...and all six factories still resolve to the projections.
      Assert.Equal("Text", Text().Description);
      Assert.Equal("Decimal", Decimal().Description);
      Assert.Equal("Integer", Integer().Description);
      Assert.Equal("Double", Double().Description);

...
namespace Unrect.Tests.Projections
{
  ...
  /// read would be enough to turn a broken file into an absent section under <c>.Optional()</c>.
  /// And a <c>Repeat</c>'s item must never defer at all (rule 2): a repeat stops when its item's
  /// placement fails, and a failure that arrives after the item has been collected is not a
  /// stopping condition, it is an exception.
  /// </para>
  /// </summary>
  public class LazyErrorTimingTests
  {
    ...
    {
      // The genuinely deferred case, and the one rule 3 is written for. The projection reads
      // nothing, so lazily nothing forces the bound until the engine consumes it in full AFTER
      // Project has returned — later than any failure could previously arrive. It still has to be
      // the same one.
    ...
    public void ABreakInsideAFlowChildCarriesTheChildsPathBothWays()
    {
      ...
      Assert.Equal("VerticalFlow -> 'body' (Range)", Failure(projection, eager: false).Path);
    }
    ...
    public void AnIoFaultInADeferredScanIsNotAbsorbedByAToleranceBoundary(string boundary, bool eager)
    {
      ...
      var failure = Failure(projection, eager);
    ...
    public void AnAbsorbableBreakInADeferredScanIsStillAbsorbed(string boundary, bool eager)
    {
      ...
      // tolerate either. The warning names the projection that failed, not the boundary that caught
      // it.
      var broken = Range(RowsWhileAny(BreaksOn(LateMarker)), _ => 0);
      var projection = boundary == "Optional" ? broken.Optional() : broken.Else(-1);

      var result = Result(projection, eager);

...
{
  /// <summary>
  /// The three example workbooks, parsed by the projections the shipped scripts declare. These are
  /// the tests that would notice if any layer changed its meaning — adapter, strategy, engine or
  /// view — so they assert real content and real extents rather than counts alone.
  /// </summary>
  public class ProjectionExampleTests
  {
    ...
    {
      // Cross-region correlation is post-parse validation, not decomposition: the projection
      // declares the document, and the caller checks that the document is internally consistent.
      var report = InvestorSummary().Map(Workbook("investor-summary.xlsx", "Summary"));
    ...
    // --- investor-irr.xlsx: one projection, two placements of the same repeat -----------------------------------------
    //
    ...
    {
      // An empty diagnostic list IS the full-consumption assertion: anything the projection left
      // over on either axis would arrive as the unconsumed-space Info. So the sheet's own
      // dimensions and the absence of diagnostics together say the declaration described all 6x45
      // of it.

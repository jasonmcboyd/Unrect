...
namespace Unrect.Tests.Projections
{
  ...
  public class DiagnosticsTests
  {
    ...
    public void BothEntryPointsStillThrowForAnUndeclaredFailure()
    {
      ...
      Assert.Throws<ProjectionException>(() => Title().Map(Square()));
      Assert.Throws<ProjectionException>(() => Title().MapWithDiagnostics(Square()));
    }
    ...
    {
      var projection = Title().Optional();

      var first = projection.MapWithDiagnostics(Square());
      var second = projection.MapWithDiagnostics(Square());
    ...
    [Fact]
    public void AProjectionThatDescribesTheWholeSpace_ReportsNothing()
    {
    ...
    public void RowsLeftOver_AreReportedWithTheFirstUndescribedCell()
    {
      ...
      Assert.Equal("the projection consumed 1 of 2 rows; rows 2+ were not described", info.Message);
    ...
    public void ColumnsLeftOver_AreReportedWithTheFirstUndescribedCell()
    {
      ...
      Assert.Equal(DiagnosticSeverity.Info, info.Severity);
      Assert.Equal("the projection consumed 1 of 2 columns; columns 2+ were not described", info.Message);
    ...
    [Fact]
    public void SpaceSkippedBeforeAProjection_IsUndescribedToo()
    {
      // A projection that starts two rows down described neither the rows it skipped nor the rows
    ...
    public void AnAbsorbedRootReportsItsWarningAndNothingElse()
    {
      ...
      // named the projection, the reason, and the cell, saying the same thing more vaguely.
    ...
    {
      // Suppression is only for a root that read nothing at all; a projection that read some of the
    ...
    public void TheRecoveryRecipe_StopsCleanlyWhenTheAnchorsRunOut()
    {
      ...
      Assert.Equal("the projection consumed 2 of 3 rows; rows 3+ were not described", info.Message);
    }

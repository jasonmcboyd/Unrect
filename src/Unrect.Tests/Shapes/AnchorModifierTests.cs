...
{
  /// <summary>
  /// The three anchors — <c>.On</c>, <c>.Below</c>, <c>.RightOf</c> — which say where a shape starts
  /// by naming something in the grid rather than a distance to it. They are one-liners over
  ...
  public class AnchorModifierTests
  {
    ...
    // --- On: the shape lands on the match and owns it -------------------------------------------------

    [Fact]
    public void On_LandsOnTheMatchedRowAndOwnsIt()
    {
      // Owning it is the whole difference from Below: the matched row is inside the extent, which is
      // how a caption becomes content the shape reads rather than a gap it steps over.
    ...
    {
      // The second child's band is the single column to the right, one cell of which is the landmark.
      // Read as "the next band along this flow", there would be nowhere to go and this would fail.
    ...
    [Fact]
    public void AnAnchorReplacesWhateverOffsetTheShapeAlreadyHad()
    {
    ...
    [Fact]
    public void AnAnchorDiscardsAShapesDefaultOffset()
    {
    ...
    public void ADeclaredAreaSurvivesAnAnchor()
    {
      ...
      // .Sized is the other half's own replace — so an extent the shape declared is still its
    ...
    {
      // Message identity, not merely a shared phrase: the renovation moved the spelling and must not
      // have moved the diagnostic. A matcher only locates and reports absence, and it reports the
      // same absence whether a modifier or a raw lift asked it.
    ...
    [Fact]
    public void TheAnchorModifiersRejectANullShape()
    {
      Assert.Equal("shape", Assert.Throws<ArgumentNullException>(() => ((IShape<string>)null!).On(Detail())).ParamName);
      Assert.Equal("shape", Assert.Throws<ArgumentNullException>(() => ((IShape<string>)null!).On(DetailColumn())).ParamName);
      Assert.Equal("shape", Assert.Throws<ArgumentNullException>(() => ((IShape<string>)null!).Below(Detail())).ParamName);
      Assert.Equal("shape", Assert.Throws<ArgumentNullException>(() => ((IShape<string>)null!).RightOf(DetailColumn())).ParamName);
    }

    private static string Miss(IShape<string> shape, ISpace space)
      => Assert.Throws<ShapeException>(() => shape.Map(space)).Message;

    private static void AssertSameOffset(IShape<string> lifted, IShape<string> anchored, ISpace space)
    {

...
namespace Unrect.Tests.Shapes
{
  ...
  public class FlowShapeTests
  {
    ...
    public void AChildWithoutADeclaredArea_ConsumesOnlyWhatItsContentUsed()
    {
      ...
      var shape = VerticalFlow(v => $"{v.Next(VerticalFlow(w => $"({w.Next(IntCell())},{w.Next(IntCell())})"))}|{v.Next(IntCell())}");
    ...
    public void AFlowHasNoArityLimit()
    {
      ...
      var shape = VerticalFlow(v => string.Join(",", new[]
      {
      ...
      var applied = shape.Apply(Ladder(12));
    ...
    public void UserCodeThrowingBetweenNextCalls_IsWrappedOnceAtTheFlowsOwnOrigin()
    {
      ...
      var failure = Assert.Throws<ShapeException>(() =>
        VerticalFlow<int>(v => { _ = v.Next(IntCell()); throw new InvalidOperationException("boom"); }).Map(Ladder()));
    ...
    {
      var failure = Assert.Throws<ShapeException>(() =>
        VerticalFlow<int>(v => { _ = v.Next(IntCell()); throw new NullReferenceException("boom"); })
          .Optional()
          .Map(Ladder()));
    ...
    public void AnArgumentExceptionBetweenNextCalls_IsStillAbsorbable()
    {
      ...
      var result = VerticalFlow<int>(v => { _ = v.Next(IntCell()); throw new ArgumentException("bad"); })
        .Optional()
        .MapWithDiagnostics(Ladder());
    ...
    public void AFlowThatDeclaresNothing_Fails()
    {
      ...
      var failure = Assert.Throws<ShapeException>(() => VerticalFlow(_ => 42).Map(Ladder()));
    ...
    public void AFlowThatDeclaresNothing_ResistsAToleranceBoundary()
    {
      ...
      Assert.Throws<ShapeException>(() => VerticalFlow(_ => 42).Optional().Map(Ladder()));
    }
    ...
    public void ANullFirstChildIsReportedAtTheFlowsOrigin()
    {
      ...
      var failure = Assert.Throws<ShapeException>(() => VerticalFlow(v => $"{v.Next(missing!)}").Map(Ladder()));
    ...
    public void AFlowHasNoChildrenToEnumerate()
    {
      ...
      Assert.Empty(VerticalFlow(v => $"{v.Next(IntCell())}{v.Next(IntCell())}").Children);
    }
    ...
    public void AFlowSaysWhyItsChildrenAreMissing()
    {
      ...
      var marker = Assert.IsAssignableFrom<IOpaqueComposite>(shape);
    ...
    public void AFlowIsAShapeAndCanBeNamedAndPlaced()
    {
      ...
      var shape = VerticalFlow(v => $"{v.Next(IntCell())}|{v.Next(IntCell())}").AfterBlankRows().Named("block");
    ...
    //
    //   IShape<List<int>> s = VerticalFlow(v => Enumerable.Range(0, 3).Select(i => v.Next(x)).ToList());
    ...
    //
    //   IShape<int> s = VerticalFlow(v => { int F() => v.Next(x); return F(); });
    //     CS9108 — the same rule for a local function.
    //
    //   IShape<LayoutCursor> s = VerticalFlow(v => v);
    ...
    //
    //   VerticalFlow(header);   // one shape, no lambda

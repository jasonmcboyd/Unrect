...
{
  /// <summary>
  /// How the table and field factories in <see cref="Projection"/> actually read a region: caption
  ...
  public static partial class Projection
  {
    private static IReadOnlyList<T> BindRows<T>(TableView table, RowBinding<T> plan)
    {
      ...
      // Streamed rather than indexed: the columns are settled from the header above, and from here
      // on the table is read forward-only, one row per step. On an extent whose height is
      // discovered as it is read that is the difference between one pass over the sheet and two.
      var rows = new List<T>();
    ...
    private static IReadOnlyList<IReadOnlyDictionary<string, CellValue>> DictionaryRows(TableView table)
    {
      ...
      // Grown rather than pre-sized, for the reason BindRows gives; the slack goes back the same
      // way.
      rows.TrimExcess();

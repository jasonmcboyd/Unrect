...
{
  /// <summary>
  /// Whether a reference-typed member was declared <c>string?</c>. <c>Nullable&lt;T&gt;</c> is a
  /// CLR type and reflection sees it; an annotated reference type is not — it is the plain type
  /// plus metadata the consumer's compiler emitted, and reading that metadata is what makes the
  /// declared type mean what it says.
  /// <para>
  ...
  internal static class NullableAnnotations
  {
    ...
    /// <c>NullableAttribute</c> on a type describes its base type's and interfaces' nullability,
    /// and reading it as the scope default would let that shadow the type's real context.
    /// </summary>

...
namespace Unrect.Projections
{
    ...
    where TSpace : class, ISpace
  {
    ...
    /// <c>VerticalFlow(v =&gt; new Report(Header: v.Next(header), Rows: v.Next(rows)))</c>. The
    /// parts are named where they are read, and there is no arity to run out of.
    /// <para>
    /// The lambda declares a <em>sequence of projections</em>, nothing more. Alternation belongs to
    /// <c>Choice</c>, <c>Else</c>, and <c>Optional</c>; repetition to
    /// <c>VerticalRepeat</c>/<c>HorizontalRepeat</c>; gaps to the following projection's offset.
    /// Conditionals, loops, and arithmetic over positions inside the lambda are the row-walking
    /// this library exists to replace, and a lambda that picks a later projection from an earlier
    /// value can never be rendered or checked without a file.
    /// </para>
    /// <para>
    ...
    /// to a list gives that up. It also runs partially inside a losing <c>Choice</c> branch, where
    ...
    /// anything and describe nothing — and what it declares can be enumerated only by running it,
    /// so a flow cannot be inspected without a space.
    /// </para>
    /// </summary>
    public static IProjection<TSpace, T> VerticalFlow<T>(Layout<TSpace, T> build)
      => new FlowProjection<TSpace, T>(Orientation.Vertical, NotNull(build, nameof(build)), Placement.Default);

    /// <summary>
    /// A flow rightwards, whose children are declared by calling <c>Next</c> on the cursor; see
    /// <see cref="VerticalFlow{T}(Layout{TSpace, T})"/> for what belongs in the lambda and what does not,
    /// including why children belong in well-named locals rather than written inline.
    /// </summary>
    public static IProjection<TSpace, T> HorizontalFlow<T>(Layout<TSpace, T> build)
      => new FlowProjection<TSpace, T>(Orientation.Horizontal, NotNull(build, nameof(build)), Placement.Default);
    ...
    /// order: <c>Overlay(o =&gt; new Header(Entity: o.Next(entity), Funds: o.Next(fundBand)))</c>.
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The lambda's rules are a flow's rules: see <see cref="VerticalFlow{T}(Layout{TSpace, T})"/> for what
    /// belongs in it, why it must capture nothing it writes to, and why parsing belongs in the
    /// leaf. Hoist each child into a well-named local here too — an overlay's children are anchored
    /// to their own content rather than ordered, so the name is often all a reader has to tell them
    /// apart. It must call <c>Next</c> at least once.
    /// </para>
    /// </summary>
    public static IProjection<TSpace, T> Overlay<T>(Layout<TSpace, T> build)
      => new OverlayProjection<TSpace, T>(NotNull(build, nameof(build)), Placement.Default);
  }
}

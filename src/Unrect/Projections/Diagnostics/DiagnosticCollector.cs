...
namespace Unrect.Projections
{
  ...
  {
    private List<ProjectionDiagnostic>? _diagnostics;
    ...
    public void Add(ProjectionDiagnostic diagnostic) => (_diagnostics ??= new List<ProjectionDiagnostic>()).Add(diagnostic);

    /// <summary>
    /// Whether everything recorded since <paramref name="mark"/> is one absorbed failure — a
    /// projection that failed, was tolerated, and produced nothing else to say.
    /// </summary>
    ...
    public IReadOnlyList<ProjectionDiagnostic> Snapshot()
      => _diagnostics is null ? Array.Empty<ProjectionDiagnostic>() : _diagnostics.ToArray();
  }
}

...
{
  /// <summary>
  /// Where a projection landed, in 1-based coordinates relative to the space <c>Map</c> was called
  /// with.
  /// </summary>
  public readonly struct ProjectionLocation
  {
    internal ProjectionLocation(int row, int column, Size available)
    {
      Row = row;
      Column = column;
      Available = available;
    ...
    /// <summary>The form every <see cref="ProjectionException"/> and <see cref="ProjectionDiagnostic"/> message uses.</summary>

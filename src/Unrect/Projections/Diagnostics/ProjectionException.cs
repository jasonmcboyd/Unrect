...
{
  /// <summary>
  /// The one exception the fused projection layer throws. It names the projection that failed, the
  /// path that reached it, and where on the sheet it was looking; a bare <see
  /// cref="OutOfBoundsException"/> never escapes a <c>Map</c> call.
  /// </summary>
  public sealed class ProjectionException : Exception
  {
    internal ProjectionException(
      string subject,
      string problem,
      string path,
    ...
    private ProjectionException(string problem, ProjectionException original)
      : this(
        original.Subject,
        problem,
        original.Path,
    ...
    /// <summary>The quoted name or description of the projection that failed.</summary>
    ...
    /// <summary>
    /// The same failure with something added to its problem — for context only the projection above
    /// it could know. The subject, path, and location are untouched: whatever failed still owns the
    ...
    /// </summary>
    internal ProjectionException WithNote(string note)
      ...
      => new ProjectionException($"{Problem.TrimEnd('.')}; note: {note}", this);

    private static string BuildMessage(string subject, string problem, string path, ProjectionLocation location)

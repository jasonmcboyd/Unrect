...
namespace Unrect.Projections
{
  ...
  /// decides the boundary: the rule is the strategy's own, written before any data was seen. Only
  /// the moment it runs has moved.
  /// <para>
  ...
  internal sealed class BoundedSpace : ISpace, ISpaceChart
  {
    ...
    /// of numbers, so there is no answering half of it: the width alone is settled and free, but
    /// only to a caller holding the scan — <see cref="WidthOf"/>, which the views use and no public
    /// caller can. That is why a strategy that must not force reads cells rather than extents —
    ...
    public ISpace GetSubspace(Offset offset, Area area)
    {
      ...
      // Through the rows asked for and no further: a request for part of the extent is not a
      // question about the whole of it. What comes back is an ordinary subspace — its extent was
      // just named.
    ...
    /// area, since a declared area is consumed in full whether or not the projection used all of
    /// it. Idempotent, and free once anything has settled the bound.
    /// </summary>
